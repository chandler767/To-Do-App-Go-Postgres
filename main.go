...
func newEntry(w http.ResponseWriter, req *http.Request) {
	if req.Method == http.MethodPost {
		...
		if task != "" {
			...
			err := tpl.ExecuteTemplate(w, "task-entry.html", "")
			if err != nil {
...
func main() {
	...
	if err != nil {
		panic("failed to connect database:" + "   serverport=" + os.Getenv("PORT") + "   host=" + os.Getenv("PGHOST") + " port=" + os.Getenv("PGPORT") + " user=" + os.Getenv("PGUSER") + " dbname=" + os.Getenv("PGDATABASE") + " sslmode=disable password=" + os.Getenv("PGPASSWORD"))
	}
	defer db.Close()
